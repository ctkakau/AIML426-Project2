    ...
    for f, func in enumerate(funcs):
      for d, D in enumerate(dims):
        for r in range(runs):
          ...
          multirun_results = multirun_results._append({'Method' : meth.__name__, 'Benchmark': func, 'Dimensions': D, 'Run': r, 'HallOfFame': hof, 'BestFitness':  best_fit}, ignore_index=True)
...
```{python runExp}
py_res = multirun(ECM = ec_methods, funcs = benches, dims = dim_sizes, runs = 30)
```
